...
const insertTaggedInventory = async data => {
  try {
    // get versions
    const maxVersion = await pool.query('SELECT MAX(version) FROM "salesReporting".tagged_inventory')
    ...
    const numVersions = await pool.query('SELECT DISTINCT(version) AS version FROM "salesReporting".tagged_inventory ORDER BY version ASC')
