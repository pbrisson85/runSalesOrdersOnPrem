    ...
    for (key of keys) {
      ...
      if (typeof response[0] === 'undefined') {
        const queryString = `
          ...
          WHERE "Inventory Location File".ON_HAND_IN_UM <> 0 AND "Inventory Location File".LOCATION LIKE ? AND "Inventory Location File".LOT_NUMBER_OR_SIZE LIKE ? AND "Inventory Location File".ITEM_NUMBER LIKE ?` //prettier-ignore
        ...
        if (typeof response[0] === 'undefined') {
          ...
          await logEvent({
            event_type: 'error',
            funct: 'getLotCosts',
            reason: 'No inventory found - Alternative failed',
  ...
  try {
    ...
    for (line of data) {
      ...
      */
      ...
      const queryString_1 = `
        SELECT {fn RTRIM("Inventory Location File".ITEM_NUMBER)} AS ITEM_NUMBER, "Inventory Location File".ON_HAND_IN_UM, "Inventory Location File".LOT_AVERAGE_WEIGHT, "Inventory Location File".LAST_COST 
        
        FROM 'Inventory Location File' 
        
        WHERE "Inventory Location File".ON_HAND_IN_UM <> 0 AND "Inventory Location File".ITEM_NUMBER = ?` //prettier-ignore
